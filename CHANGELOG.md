...
### B6 运营闭环：把"造好但没接线"的能力接上入口（2026-08-13，分支 `feature/b6-ops-closed-loop`）

八项改动的共同点是——能力本来就有、或数据本来就在，缺的是最后一段链路。

- **评测链路打通**：`capability/eval` 下的 Runner/JudgeModel 此前全仓除自身单测外无任何调用方。新增
  `EvalRun`（运行快照，两类评测的指标归一到 0-1）/ `EvalComparison`（版本对比）/ `EvalRunStore` SPI 四件套 +
  `EvalService` 编排（跑标准集 → 落库 → 对比上一版）+ `EvalBaselineScheduler` 定时基线 + `JudgeModel` 默认装配。
  三个入口：app-server `/api/customer/eval/**`（X-API-Key，供 CI 门禁）、定时任务、后台"立即评测"。
  **对比同时给两个视角**：`verdict`（总分方向）与 `regressions`（上版通过、这版失败）——准确率涨 2 个点的同时
  弄坏一个用例，是最容易被总分掩盖、也最该被看见的情形。
  取基线按自增 `seq` 而非 `created_at_ms`：评测是纯内存计算，连跑两次会落在同一毫秒，按时间戳取不到基线。
- **badcase 回流闭环**：负反馈与质检失败此前只写 `cw_fact_log` 就结束了。新增 `cw_badcase` 待筛队列
  （与事实流水并存：流水答"当时发生了什么"、队列答"我们拿它做了什么"，把状态塞进只追加的审计流水会破坏其约定）+
  `BadcaseService`（转知识库 / 转评测用例，**两者不互斥**：补知识治本、加用例防复发）+ admin 筛选界面。
  前置补齐 `cw_eval_case`：评测集原本是 jar 内只读 JSON，"转成评测用例"无处落地，飞轮最后一环是断的。
  登记时从 `cw_chat_message` 回查一问一答——只给运营一个 messageId 根本无从判断该不该回流。
  **刻意不做自动回流**：把最不满那批用户的反馈直接灌进知识库是一个投毒面。
- **语义缓存**（全仓此前 0 命中）：`SemanticCacheService` 按问题向量相似度复用上次答案，命中即省掉整条链路。
  **默认关闭**，三道安全闸门收口在 `cacheable` 一处：意图白名单（默认仅 `consult`）+ 个人标识过滤
  （问题或答案含 6 位以上连续数字即跳过）+ 双层隔离——两个用户都问"我的订单到哪了"，语义高度相似
  但答案完全不同，无差别缓存是数据泄露而不只是体验问题。
- **模型分级路由**：`TieredRoutingModel` 按难度选档（`FailoverModel` 只回答"主模型挂了用谁"）。
  **框架默认关闭**（示例应用 `application.yml` 与语义缓存等本批能力一样显式开启）。
  能力取两档交集、上下文取较小值（路由是动态的）；判定保守，只有单轮且简短才降级；
  经济档失败仅在首分片之前回退（已上屏的文字后面接主模型的完整回答会变成错乱两段）。
- **提示词版本与效果归因**：`cw_prompt_version` 记"运行时实际生效"的那份（B4 的 `ai_config_version` 记的是
  "发布了什么"，灰度未覆盖或推送未达时两者不一致）。版本号取**内容指纹**而非外部版本号。
  `EvalComparison#promptChanged` 是归因支点：指标掉了先看这一位，没变就别再对着提示词逐字找原因。
- **CSAT 会话级满意度**（此前只有消息级点赞/点踩）：`cw_csat_survey` 邀请与回收分开记——只记评分算不出
  回收率，而回收率低时那个漂亮的分数只代表愿意评价的一小撮人。满意按 4 分及以上算（行业口径）。
  会话结束自动邀请（幂等，避免多次结束把分母灌水）。
- **知识盲区分析**：检索未命中时记一笔，攒出"哪些问题反复查不到"的排行（计数表而非流水表）。
  顺带把两个知识后端各自硬编码的未命中文案提成接口契约 `KnowledgeBackend.NO_HIT_REPLY` + `isMiss`——
  否则改文案会让埋点静默失效，且没有任何测试会发现。
- **死信/重试队列**（全仓此前 0 命中）：`cw_dead_letter` + 指数退避重投 + 按类型分发的 `DeadLetterHandler` SPI +
  定时巡检。次数耗尽转 `ABANDONED` 而**不删除**（静默丢弃正是现状的问题）；无 handler 的类型跳过且不累计次数。
- **顺带修的既有问题**：`QualityEvalRunner` 用了 `log.warn`（违反项目"只用 info/error"）；
  `JudgeModel` 的 javadoc 示例引用了 2.0 GA 已不存在的 `ChatResponse::merge`/`toMsg()`，照抄编译不过。
- **后台界面**：新开一级菜单「运营闭环」（`/ops`）统领 7 个页面——评测中心（趋势图 + 回归项红框）、
  badcase 回流（双出口表单）、语义缓存（按命中次数排序 + 定点/整片清除）、提示词版本（两版全文比对）、
  满意度看板（CSAT 与回收率并列，回收率过低时明确提示"这个分数不可尽信"）、知识盲区（排行 + 一键补知识）、
  死信队列（三态计数 + 人工重开）。五个域共用一个跨库门面与连接池（`OpsGatewayProvider`）——
  同属一批、都是低频运营查询，各开一个池只是白占连接数。

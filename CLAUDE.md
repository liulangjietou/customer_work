...
- 2026-09-23 循环守卫去向说明修复（无迁移，客服端仍从 **V31**、Admin 从 **V115** 核对）：全模块
  ...
  **合计 4942**（排除 `RedisSessionPersistenceTest`）。本批次自身加 starter **+9**
  （`LoopGuardRealAgentStreamTest` 4 + `LoopGuardMiddlewareTest` 5）；admin 比上一版多 1 条来自其间合入的其它改动。两条经验：
  ① **中间件补给流式消费方的正文，要落在文本块生命周期之内、块结束之前**：框架的收尾回复本身逐片流式，
...
- 2026-09-15 Badcase 与词表批量操作批次新增客服端 **V30**，仅将敏感词表租户身份列改为
  ...
  `docs/admin-experience/adoption-actions-validation.md`。

- 2026-09-15 个人草稿受控试用新增 Admin **V112/V113**，下一 Admin 从 **V114** 核对；客服端仍从 **V30** 核对。
  试用先持久受理、独立只读执行、按原标识恢复，统计标为 `EVALUATION`；发布检查只读聚合现有门禁与实例 ACK。
  V113 仅对齐表默认值和普通文本列，保留已执行 V112 的校验和以及身份列大小写语义，不使用整表 CONVERT。
  执行范围、真实模型与升级验证见 `docs/admin-experience/agent-draft-trial-validation.md`。

- 2026-09-14 知识候选闭环新增 Admin **V108–V111**、客服端 **V29**；下一 Admin 从
  **V112**、客服端从 **V30** 核对活动分支与共享库后分配。候选只读对照复评绑定冻结的模型、
  提示词、语料和审核用例，答复及 Judge 调用统一记为 `EVALUATION`；旧 `/fill` 直写入口已关闭。
  发布结果未知时保留原任务，由 Worker 先核对客服库回执再恢复，禁止重新生成任务重复插入 FAQ。
  复评使用执行编号与总截止时间隔离迟到结果，发布成功仍不表示线上效果已验证。迁移、镜像、
  双库恢复与验证范围见 `docs/admin-experience/knowledge-candidate-workflow.md`。

- 2026-09-12 客户消息原文与同步撤权新增客服端 **V28**，Admin 仍为 **V106**。
  旧知识投影默认 `BLOCKED`，管理员须显式重新投影所需版本；正文检索与预览均要求当前 `READY`。
  KB/source 权限变更先在 KB 行锁内同步提交客服库封锁，封锁失败阻止后台写入；后台回滚不会恢复公开状态。
  下一客服迁移从 **V29**、下一 Admin 从 **V107** 核对活动分支与共享库后分配。
  真实双库事务、客户 HTTP、迁移与浏览器验证范围见 `docs/admin-experience/customer-source-preview-validation.md`。
...
- 2026-09-11 未命中来源与分类复核新增客服端 **V25 / V26**（分类复核及通用审计时间），下一版本从 **V27** 核对；
  Admin 保持 **V104**，下一版本从 **V105** 核对。分类 CAS 与复核流水在客服端同库提交，
  工具和自动注入两条路径均在异步调度前冻结租户与来源。契约及验证见
  `docs/admin-experience/knowledge-gap-classification-contract.md`。

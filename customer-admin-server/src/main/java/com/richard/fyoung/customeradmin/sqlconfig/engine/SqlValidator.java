...
/**
 * 只读 SQL 校验器：保存 SQL 定义时调用一次（全链路唯一的 SQL 文本防御点，fast-fail）。
 *
...
public final class SqlValidator {
    ...
    public static void validateReadOnly(String sql) {
        ...
        if (stripped.contains(";")) {
            throw new BizException(ResultCode.SQL_NOT_READONLY, "SQL 不允许多语句（含分号）");
        }
        String firstWord = firstWord(stripped);
        if (!"SELECT".equalsIgnoreCase(firstWord) && !"WITH".equalsIgnoreCase(firstWord)) {
            throw new BizException(ResultCode.SQL_NOT_READONLY, "仅允许只读 SELECT/WITH 查询，当前首关键字: " + firstWord);
        }
    }

...
test('当前终态按消息标识取来源，保存未确认与传输结束均不冒充无命中', async ({ page }) => {
  ...
  await reader.getByRole('button', { name: '关闭本轮检索参考' }).click()
  // fill 不检查输入框是否仍被抽屉遮挡；须等关闭与焦点恢复完成，才能模拟下一次用户输入。
  await expect(reader).toHaveCount(0)
  await expect(page.getByRole('button', { name: '本轮检索参考', exact: true })).toBeFocused()
  ...
  await expect(page.locator('.chat-panel .message-row.assistant')).toHaveCount(2)

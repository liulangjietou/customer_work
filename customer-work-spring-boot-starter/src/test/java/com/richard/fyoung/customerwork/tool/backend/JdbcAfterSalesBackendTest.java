...
/**
 * JDBC 售后后端测试（对接本机 MySQL；不可达自动跳过）：退款资格联查 cw_order、工单落库往返、种子进度查询。
 *
 * <p>写操作用例统一使用随机订单号，finally 删除自建的 cw_refund / cw_invoice_request 行，绝不污染种子数据。</p>
 * @author owlzhangfq@gmail.com
 */
class JdbcAfterSalesBackendTest {

    private static final String HOST = "localhost";
    private static final int PORT = 3306;

    private DataSource dataSource;
    private JdbcAfterSalesBackend backend;

    @BeforeEach
    void setUp() {
        assumeTrue(reachable(HOST, PORT), "MySQL 不可达（" + HOST + ":" + PORT + "），跳过该测试");
        dataSource = buildDataSource();
        // 售后退款资格 / 价保联查 cw_order，先建表并写入订单种子
        new JdbcOrderBackend(dataSource);
        backend = new JdbcAfterSalesBackend(dataSource);
    }
    ...
    private DataSource buildDataSource() {
        ...
        ds.setPoolName("test-aftersales-pool");
        return ds;
    }
    ...
    @Test
    void submitRefund_shouldGeneratePendingTicket() throws Exception {
    ...
    @Test
    void submitReturn_shouldGenerateReturnTicket() throws Exception {
        String orderId = "T-" + UUID.randomUUID().toString().substring(0, 8);
        try {
    ...
    @Test
    void submitExchange_shouldGenerateExchangeTicket() throws Exception {
        String orderId = "T-" + UUID.randomUUID().toString().substring(0, 8);
        try {
    ...
    @Test
    void requestInvoice_shouldPersistRequest() throws Exception {
        String orderId = "T-" + UUID.randomUUID().toString().substring(0, 8);
        try {
    ...
    private void deleteRefundByOrder(String orderId) throws Exception {
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement("DELETE FROM cw_refund WHERE order_id = ?")) {
            ps.setString(1, orderId);
            ps.executeUpdate();
        }
    }

    private void deleteInvoiceByOrder(String orderId) throws Exception {
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement("DELETE FROM cw_invoice_request WHERE order_id = ?")) {
            ps.setString(1, orderId);
            ps.executeUpdate();
        }
    }

 ...
 * approve 后由强制装配的 {@link ApprovalExecutionHandler} 执行资金动作；缺失执行器只会留下
 * {@link ExecutionStatus#EXECUTE_FAILED}，绝不伪报已执行。</p>
...
public class PendingApprovalService {
    ...
    private static final long EXECUTION_LEASE_TIMEOUT_MS = 5 * 60 * 1000L;

    private final ApprovalStore store;
    /** 真实执行器无默认空实现：缺失时必须失败，不能把资金动作标成已完成。 */
    private final AtomicReference<ApprovalExecutionHandler> executionHandler =
        new AtomicReference<>(PendingApprovalService::missingExecutionHandler);
    ...
    public PendingApprovalService(ApprovalStore store,
        ...
        this.store = store;
        ApprovalExecutionHandler handler = handlerProvider.getIfAvailable();
        if (handler != null) {
            executionHandler.set(handler);
        }
    }
    ...
    /** 人工放行：原子推进决策并取得执行租约后调用真实资金执行器。 */
    public ApprovalRequest approve(String id, String operator) {
        require(id);
        if (!store.decide(id, ApprovalStatus.APPROVED, operator, null, System.currentTimeMillis())) {
            throw alreadyDecided(id);
        }
        log.info("approval approved: id={}, operator={}", id, operator);
        executeApproval(id, Integer.MAX_VALUE);
        return require(id);
    }
     ...
     */
    private boolean executeApproval(String id, int maxAttempts) {
        String fencingToken = UUID.randomUUID().toString();
        long startedAtMs = System.currentTimeMillis();
        if (!store.claimExecution(id, maxAttempts, startedAtMs, fencingToken)) {
            return false;
        }
        ...
        try {
            executionHandler.get().execute(new ApprovalExecutionContext(req, req.getId(), fencingToken));
            success = true;
        } catch (Exception e) {
        ...
        if (!store.completeExecution(id, fencingToken, success, failureReason)) {
            log.error("approval execution completion fenced, errorCode={}, id={}, token={}",
                "APPROVAL-EXEC-FENCED", id, fencingToken);
            return false;
        }
        return true;
    }
     ...
     *
     * <p><b>幂等性约定</b>：{@link ApprovalExecutionContext#idempotencyKey()} 固定为审批单号，
     * 下游必须持久化消费记录并按它幂等；fencing token 只用于阻止旧 worker 回写本地执行状态。</p>
     *
     * @param maxAttempts 最大执行尝试次数（含首次执行）；&lt;=1 表示不重试
     * @return 本次实际重试的审批单数
     */
    public int retryExecutionFailures(int maxAttempts) {
        store.recoverStuckExecutions(System.currentTimeMillis() - EXECUTION_LEASE_TIMEOUT_MS);
        if (maxAttempts <= 1) {
            return 0;
        }
        List<ApprovalRequest> candidates = store.findByStatus(ApprovalStatus.APPROVED).stream()
            .filter(r -> r.getExecutionStatus() == ExecutionStatus.EXECUTE_FAILED
                || r.getExecutionStatus() == ExecutionStatus.NOT_APPLICABLE)
        ...
        for (ApprovalRequest req : candidates) {
            ...
            if (executeApproval(req.getId(), maxAttempts)) {
                retried++;
            }
        }
        return retried;
    ...
    public ApprovalRequest deny(String id, String operator, String note) {
        require(id);
        if (!store.decide(id, ApprovalStatus.DENIED, operator, note, System.currentTimeMillis())) {
            throw alreadyDecided(id);
        }
        ApprovalRequest req = require(id);
    ...
    public void onApprove(Consumer<ApprovalRequest> callback) {
        executionHandler.set(callback == null
            ? PendingApprovalService::missingExecutionHandler
            : context -> callback.accept(context.request()));
    }
    ...
    private IllegalStateException alreadyDecided(String id) {
        ApprovalRequest request = require(id);
        return new IllegalStateException(
            "approval already decided: id=" + id + ", status=" + request.getStatus());
    }

    private static void missingExecutionHandler(ApprovalExecutionContext context) {
        throw new IllegalStateException("approval execution handler not configured");
    }

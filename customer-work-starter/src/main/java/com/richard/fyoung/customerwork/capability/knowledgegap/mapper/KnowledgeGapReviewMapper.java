...
public interface KnowledgeGapReviewMapper extends BaseMapper<KnowledgeGapReviewDO> {
    ...
    @Select("""
        ...
        WHERE BINARY tenant_id = BINARY #{tenantId} AND BINARY scope_id = BINARY #{scopeId}
          AND BINARY question_hash = BINARY #{questionHash} AND revision < #{beforeRevision}
        ORDER BY revision DESC LIMIT 20
        """)
    List<KnowledgeGapReviewDO> history(@Param("tenantId") String tenantId, @Param("scopeId") String scopeId,

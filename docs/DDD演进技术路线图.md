...
| v0.8 | 2026-07-18 | 新增配套《DDD-PR演进计划表》（B0~B9 拆解为 14 个 PR + 三泳道 + 启动条件清单 + 执行 SOP），作为 T0 出站后的直接执行依据；本路线图批次调整时计划表同轮改版 |
...
- **进入开发的唯一闸门**：你明确确认"文档没问题"。在此之前所有批次（含 B0）都不动工；出站后的执行级依据见《DDD-PR演进计划表.md》（14 个 PR、分支命名、前置闸门、状态跟踪）；

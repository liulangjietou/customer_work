...
| CHAT-01 | 受理回执、同标识重试与对账、失败保留输入 | 待实施 | 落库失败/回执丢失/并发重复/内容冲突 |
| CHAT-02 | 权威终态、历史分轮、原文与运行指引分离 | 历史分轮、原文分离、SSE 协议检查已实现；实时权威终态待实施 | EOF 不伪完成，停止/错误/刷新/旧记录 |
| CHAT-03 | 双端重连补消息与工单快照 | 待实施 | WS/HTTP 对账，断线时消息及工单状态变化 |
...
| H5-01 | 接收状态、来源、业务结果、转人工与输入区 | 待实施 | 360/390px、弱网、附件、焦点及软键盘 |
...
- 当前阶段 Admin 单测 215/215、H5 单测 38/38、两端类型与构建通过；历史浏览器定向回归 8/8 通过。提交前以最终源码重新执行完整门禁，并单独记录结果。

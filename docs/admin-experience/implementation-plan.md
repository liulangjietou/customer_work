...
| DESK-01 | 队列、会话、交接摘要和建议草稿一屏协作 | 基础工作台已通过完整门禁；既有摘要的租户、来源及失败语义已修正；Admin 辅助入口与建议采用待实施 | 接单/转接竞争，摘要权限，采用不自动发消息 |
...
| `/ticket/user-ticket` | `@/views/ticket/UserTicketManage.vue` | 基础工作台、360/390px 交互和完整门禁通过；摘要待实现 |

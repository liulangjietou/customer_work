...
| CHAT-01 | 受理回执、同标识重试与对账、失败保留输入 | 客户 WS、H5 与坐席 HTTP 已实现并通过完整门禁；工作区受理待接 | 落库失败/回执丢失/并发重复/内容冲突 |
| CHAT-02 | 权威终态、历史分轮、原文与运行指引分离 | 历史分轮、原文分离、实时终态与保存核对已实现并通过回归；完整持久对账待受理记录接通 | EOF 不伪完成，停止/错误/刷新/旧记录 |
| CHAT-03 | 双端重连补消息与工单快照 | H5 与坐席端均已实现首连、重连、多页补拉和工单快照，并通过完整门禁 | WS/HTTP 对账，断线时消息及工单状态变化 |
| DESK-01 | 队列、会话、交接摘要和建议草稿一屏协作 | 基础工作台、交接摘要、原文依据与建议草稿已通过完整门禁 | 接单/转接竞争，摘要权限，采用不自动发消息 |
| KB-01 | 授权来源预览、版本追溯与失效提示 | 后台版本原文、工作区自动检索来源及客户受管 RAG 工具来源留存已通过完整门禁；H5 原文与当前撤权同步已通过完整门禁 | 引用版本、归属、跨租户/用户访问拒绝 |
| OPS-01 | 知识缺口分类、人工修正与已有改进闭环接通 | 读取事实、治理交互及来源与分类复核已通过完整门禁，知识候选继续实施 | 非知识任务分流、候选/评测/发布门禁 |
| CONFIG-01 | 场景模板、草稿、试用、评测与上线检查 | 模板、个人草稿和版本保护已通过完整门禁；受控试用与评测聚合继续实施 | 不隐式授权、不跳过发布门禁、保存契约完整 |
| H5-01 | 接收状态、来源、业务结果、转人工与输入区 | 受理、草稿与附件、答复状态、资料原文与办理进度均已实现；真实订单写入边界和物理设备验收继续实施 | 360/390px、弱网、附件、焦点及软键盘 |
...
| `/aiconfig/knowledge-base` | `@/views/aiconfig/KnowledgeBaseManage.vue` | 知识源读取和版本原文、失败/无权/失效及 390px 阅读通过；其他业务态继续验收 |
...
| `/ops/knowledge-gap` | `@/views/ops/KnowledgeGapBoard.vue` | 读取、分类复核、共享治理及 390px 交互通过；知识候选待接通 |
...
| `/ticket/user-ticket` | `@/views/ticket/UserTicketManage.vue` | 工单、摘要、原文与建议采用的业务交互及桌面/360/390px 验证通过 |
...
坐席受理与双端恢复已接通，当前基础工作台按 [坐席工作台契约](service-desk-contract.md) 验证，结果见 [工作台验证记录](service-desk-validation.md)。本批补齐工单行锁、转派时消息归属、订阅凭证用途，以及草稿、回执和阅读位置的前端状态。完整门禁通过：Maven 3974 条，0 失败、0 错误、7 跳过；Admin 247 条单测及 142 条 E2E；H5 53 条单测及 6 条 E2E；两端类型和构建通过。工作区受理、完整终态对账、交接摘要及建议采用仍待继续，第二批整体尚未完成。

交接摘要基础修复的职责、失败证据和接入边界见 [摘要基础验证](summary-foundation-validation.md)。完整门禁通过：Maven 3987 条，0 失败、0 错误、7 条环境门控跳过；Admin 247 条单测及 142 条 E2E；H5 53 条单测及 6 条 E2E；两端类型与构建通过。当前只修正既有服务及异步增强链路，工单范围内的 Admin 辅助入口和前端采用流程尚未接通。

配置模板与草稿的契约及验证范围见 [配置草稿工作流](configuration-drafts-validation.md)。这一批不将模板的验收建议标记为已执行测试，也不绕过既有渠道发布与评测门禁。

模板与个人草稿批次完整门禁通过：Maven 3995 项（0 失败、0 错误、7 项环境跳过），Admin 247 项单测与 153 项 E2E，H5 53 项单测与 6 项 E2E，两端类型和构建通过。

知识缺口读取与共享治理交互的边界和验证见 [知识缺口工作流](knowledge-gap-workflow-validation.md)。

知识缺口读取与治理交互批次完整门禁通过：Maven 4001 项（0 失败、0 错误、7 项环境跳过），Admin 247 项单测与 162 项 E2E，H5 53 项单测与 6 项 E2E，两端类型和构建通过。分类信号、人工修正留痕与知识候选仍在继续实施。

## 知识缺口来源与分类复核

两条检索路径已接入可信来源捕获；正常未命中、分类原因与人工优先级分开展示。人工复核以同库事务保存当前分类和不可变记录，并以修订比较保护并发。长问题使用截断前文本判断，可能已截断的历史信号不自动分类。完整门禁通过：Maven 全模块 4015 项（0 失败、0 错误、7 项环境门控跳过），Admin 247 项单测与 168 项 E2E，H5 53 项单测与 6 项 E2E，两端类型与构建通过。契约见 `knowledge-gap-classification-contract.md`。

## 工单辅助入口与建议采用

从当前工单的授权会话读取已有摘要或规则整理，原文可单独核对；采用只进入当前草稿，已有内容需明确追加。切单、消息更新和读取失败均保护建议与草稿。定向后端 32 项、工作台浏览器 17 项及补充长文本后的 9 项辅助用例通过，桌面与 390px 截图已查看。完整门禁通过：Maven 4023 项（0 失败、0 错误、7 项环境门控跳过），Admin 247 项单测和 177 项 E2E，H5 53 项单测和 6 项 E2E，两端类型与构建通过。契约见 [工单辅助](service-desk-assist-contract.md)。

## 后台知识版本原文

版本目录与原文预览的权限、版本归属、失效语义和定向测试见 [知识版本原文验证](knowledge-version-preview-validation.md)。聊天引用仍待接入实际持久依据。

后台原文预览批次通过 Admin 及 starter 反应堆 3,805 项（0 失败、0 错误、7 项环境跳过）、Admin 247 项单测与 186 项 E2E，类型与构建通过。按改动范围未在本地重复未改动 H5/客服应用的全套验证。

## 检索故障与未执行的统计区分

已复现并修正自动注入把检索故障或未配置知识库记为未命中的问题。实际 HTTP、部分成功与超时边界已通过定向验证；整个 Maven 反应堆 4,076 项通过（0 失败、0 错误、7 项环境条件跳过）。范围与兼容性见 [检索结果验证](knowledge-retrieval-outcome-validation.md)。


## 工作区消息检索参考进度

本轮新增后台消息的自动检索来源留存与按版本原文预览，当前与历史消息均保留稳定标识，状态和失败处理见 [工作区来源验证记录](workspace-knowledge-sources-validation.md)。Maven 全模块 4,108 项（0 失败、0 错误、7 项环境条件跳过）、Admin 255 项单测及 204 项 E2E 通过，类型和构建通过。此批不把模型文字解析为来源；工具检索和 H5 的来源仍按各自真实链路继续接入。KB-01 和整体计划均未据此全部完成。

## 客户答复状态与来源留存

客户答复正文、结束原因、计划和参考线索同次落库，受管 RAG 工具在框架格式化前记录真实来源；实时事件和历史复用同一份快照。H5 支持当前、分页与重连恢复，并区分助手计划和实际业务结果。Maven 全模块 4,121 项（0 失败、0 错误、7 项环境条件跳过）、Admin 255 项单测及 204 项 E2E、H5 68 项单测及 11 项 E2E、两端类型与构建通过。证据和边界见 [客户答复验证](customer-answer-evidence-validation.md)。H5 原文读取、当前授权失效、业务结果卡片及物理设备键盘验收仍继续实施。

## 客户消息历史原文与当前撤权

H5 消息内可打开真实留存的历史片段，并在每次请求核验当前授权。后台权限变更同步封锁客服投影，重新投影保留稳定引用。独立审查补齐默认关闭租户插件时的显式租户限制和原生工具上下文恢复，详细失败与修复证据见 [客户原文验证](customer-source-preview-validation.md)。Admin 255 项单测/204 项 E2E、H5 85 项单测/18 项 E2E及类型构建通过；修复后的完整后端 4,199 项通过（0 失败、0 错误、7 项环境条件跳过），55 个冻结文件未变动。业务进度、工作区对账、知识候选、配置试用与全站业务状态验收按剩余条目继续。

## 当前会话的办理进度

H5 可以分别核对当前工单、退款审批与执行记录，并从工单、订单返回原会话。用户会话及订单归属在真实客服数据库读取时约束；内存审批、原生工具审批登记、旧运营入口与匿名诊断保留各自的身份和状态命名空间。完整后端 4,277 项（0 失败、0 错误、7 项环境条件跳过）、Admin 255 项单测/204 项 E2E、H5 108 项单测/23 项 E2E及两端类型构建通过；两个 H5 弹层均覆盖标题点击后的反向 Tab 和 Escape。详情见 [办理进度验证](customer-business-progress-validation.md)。存量售后业务写链、工作区对账、知识候选、受控试用和逐页业务验收继续处理。

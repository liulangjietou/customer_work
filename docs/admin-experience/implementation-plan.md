...
| KB-01 | 授权来源预览、版本追溯与失效提示 | 后台版本原文、工作区自动检索来源及客户受管 RAG 工具来源留存已通过完整门禁；H5 原文与当前撤权同步继续实施 | 引用版本、归属、跨租户/用户访问拒绝 |
...
| H5-01 | 接收状态、来源、业务结果、转人工与输入区 | 受理状态、草稿与附件、答复状态及参考线索历史恢复通过完整门禁；原文和业务结果继续实施 | 360/390px、弱网、附件、焦点及软键盘 |

...
| **主动服务（复用飞书推送）** | starter `POST /api/customer/notify/order-status\|survey` | customer-web 的 `FeishuNotificationChannel`（`@Primary`）覆盖 starter 默认 `LoggingNotificationChannel`，把订单状态通知/满意度回访经飞书 webhook 推达；webhook 未配置时优雅降级为日志。即 starter 的主动服务（`ProactiveNotificationService`）在 customer-web 中自动走飞书通道 |

...
登录轮播使用独立图片区，按原比例完整展示横图和竖图，移除整幅深色遮罩，品牌标题放在图片之外。手机端保留图片区、暂停和选图入口。单张加载失败会跳过；配置图全部失败时显示内置图片，内置图也失败则回到默认品牌说明，避免持续空白或循环请求。后台轮播素材页同步说明空列表的实际展示行为。
...
| `Login` → `LoginBrandStage` | 图片显示、加载失败兜底与轮播控制 | `/login-images/list` 返回启用图地址，图片由公开接口从对象存储读取；图片故障不阻断认证 |
...
轮播补充验收：重新通过管理端 205 项单元测试、31 项完整登录回归、类型检查和生产构建。新增三项用例覆盖坏图跳过、全部失效及兜底再次失效、手机端图片与控制入口；先在旧实现确认失败再修复。另检查 981×560、1280×720、1440×960、390×844 视口及 Night 主题，图片和文字互不遮挡，无横向溢出。本机原图的 404 源于图片存储容器未运行，恢复既有容器后，两张原图均返回 200，无需替换或重新上传。

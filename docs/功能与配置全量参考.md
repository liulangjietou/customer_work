...
| 长期记忆（主体级隔离） | `LongTermMemoryProvider` + `MemorySubjectResolver` | 开 | `memory.provider=memory/bailian/mem0/reme`；prod 门禁当前只允许 `memory`，见 §6.5、§6.34 |
...
| **会话状态乐观并发（2.0.3）** | `AgentGovernanceAssembler` + `AgentStateConflictMetrics` | 开（框架默认） | `agent.state-conflict-policy=OVERWRITE/FAIL/APPEND_MERGE`，见 §6.9b |
| **最终答复过滤（2.0.3）** | 框架 `FinalAnswerFilterMiddleware` | 关 | `agent.final-answer-filter-enabled=true`。**打开会让流式打字机效果消失**，见 §6.9b |
...
| **人机切换闭环（工单系统）**：AI 转出→坐席接单→结案回收 | `HandoffService` 兼容层 → `TicketService` 唯一状态机 | 开 | `GET /handoffs` · `POST /handoffs/{id}/claim\|resolve` · `ticket.store-mode` |
| **人机切换 SLA 升级引擎** | `TicketSlaScheduler` | 开 | `ticket.sla-*`（WAITING_AGENT/PROCESSING 超时告警，确认/归档态自动流转） |
...
| **租户实时撤权与退租交付** | `TenantService` + `TenantAccessPublishWorker`（admin）+ `TenantAccessNacosService` / `TenantAccessGuard` / `WsSessionRegistry`（starter） | prod 消费端开 | 用户/租户 epoch、可靠访问快照、新请求与 WS 握手 fail-closed、冻结/退租快照应用后主动断开既有 WS；见 §6.27、§6.34 |
| **主体级长期记忆隐私** | `MemorySubjectResolver` + `GovernedLongTermMemory` + `MemoryConsentService` + `UserPrivacyController` | prod 同意门禁开 | “租户 + 已验签主体 + Agent”隔离，同意/查看/导出/撤回删除/保留清理；生产门禁强制 `provider=memory` 且记忆/同意均为 JDBC，见 §6.34 |
| **数据权限（角色数据范围，仅本人过滤）** | `DataScopeInnerInterceptor` + `DataScopeResolver`（admin） | 开 | `admin.data-scope.enabled`（默认 `true`），范围配在角色上，见 §6.33 |
| **水平扩展（限流/租户配额/主体配额/会话锁共享）** | `WindowCounter` + `SessionLock`（Redisson 实现） | 关 | `customer-work.distributed.counter-mode=redis` + `session-lock-mode=redis`，见 §6.28 |
| **FinOps：租户配额 / 账单 / 预测 / 告警 / 导出** | `TenantQuotaGuard`（starter）+ `CostForecastService` / `CostAlertService` / `BillingCsvExportService`（admin） | 关 | token `BLOCK/DEGRADE/WARN`、T+1 金额账单、线性预测、告警 ACK、CSV；见 §6.29、§6.34 |
| **配置版本化与按租户灰度** | `ConfigVersionService` + `ConfigRollbackService` + 可靠发布任务/实例 ACK（admin） | 随发布能力 | `admin.runtime-publish.nacos.enabled=true`；contentHash 变化先严格失效租户语义缓存，见 §6.30 |
| **EvalOps：数据集版本 / 评测 / release gate** | `IntentEvalRunner` + `QualityEvalRunner` + `EvalReleaseGateService` + `BadcaseService` + `PromptVersionStore` | 评测手动触发（每日基线关）；badcase 开 | 不可变用例快照、九维制品、阈值/回归/关键用例/Judge 门禁、重评/审计豁免；见 §6.31、§6.34 |
...
| **Agent 改进闭环** | `ImprovementCaseService` + `ImprovementAutomationWorker`（admin） | 自动发布跟踪与效果观察默认开 | KnowledgeGap/badcase 共用 owner、SLA、精确候选、目标用例复评、可靠发布任务及 revision 曝光/复发观察；见 §6.31、§6.34 |
...
| **ModelOps 与模型路由** | `ModelEndpointPolicy` + `ModelHealthService` + `ModelCertificationService` + `ModelRoutingPolicyService` + SecretRef | 健康巡检默认关 | 资产/部署、端点保存期出网校验、健康/认证/影响、不可变路由版本、dry-run 与真实选路；见 §6.34 |
| **在线双臂实验** | `ModelExperimentLifecycleService` + `OnlineExperimentRoutingModel` + 调用日志指标 | 按实验启动 | desired/effective state 分列、稳定分桶/归因、错误率/P95 护栏、到期/自动停止；见 §6.34 |
| **Trace / 只读 Replay** | `AgentCallTimingMiddleware` + `AgentCallReplayManifestVO` | 调用日志开启后 | 冻结 runtime/制品/实验事实，重放清单 `executable=false`；见 §6.34 |
| **SLO / error budget 与业务结果** | `SloAutomationWorker` + `SloEvaluationService` + `BusinessOutcomeService` | SLO 周期评估默认开启，也可手工触发 | 多副本数据库租约、短/长窗口 burn rate、OPEN/ACKED/RESOLVED、恢复事件、可靠通知；技术成功/自动解决代理/转人工/CSAT 会话下钻；见 §6.34 |
...
| **终端用户认证（JWT）** | `UserAccountService`(BCrypt) + `UserJwtService`(HS256) + `UserAuthWebFilter` | 开 | `customer-work.user-auth.store-mode=jdbc`（表 `cw_user`，env `CW_USER_JWT_SECRET`）；`session_epoch` 支持一键撤销全部 JWT |
...
| **用户/坐席 WebSocket 双通道** | `UserChatWebSocketHandler` / `AgentChatWebSocketHandler` + `ChatDispatchService` + `WsSessionRegistry` | 开 | `/ws/user?token=<JWT>`、`/ws/agent?token=<HMAC凭证>`；AI 流式 / 坐席转发 / 关键词转人工；冻结/退租快照应用后按租户主动断开 |
...
| POST | `/api/customer/auth/revoke-sessions` | 原子推进 `session_epoch`，立即撤销该用户全部既有 JWT（Bearer JWT） |
...
- JDK 17
- Maven 3.9 推荐；构建前端时 Node.js 24 推荐（Vite 8 最低要求 `^20.19.0 || >=22.12.0`）
...
```bash
docker compose up -d mysql                  # 默认配置依赖 MySQL
export DASHSCOPE_API_KEY=你的百炼密钥         # 必填；Maven / IDE 不会自动读取 .env
mvn -pl customer-work-app-server -am -DskipTests package
java -jar customer-work-app-server/target/customer-work.jar
```

**方式二：Docker Compose 一键起（app + Redis + MySQL + MinIO + Nacos + XXL-JOB + PaddleOCR）**
```bash
cp .env.example .env                       # 写入真实 DASHSCOPE_API_KEY，不要提交 .env
...
> `sessionId` 只标识会话，不是用户或租户身份凭据。长期记忆按“可信租户 + 已验签主体 + Agent”隔离：
> 已验签 `USER` 主体可跨自己的会话共享；其它入口按完整 `SESSION` 隔离。无请求上下文时保留前缀解析仅为
> 历史数据兼容，生产入口不能据此信任客户端自报身份。
...
```
返回 AG-UI 编码事件流，兼容 AG-UI 前端直接对接；`RUN_FINISHED.result` 与同步/SSE/WS 共用
`{messageId, finishReason, usage, traceId}` 终止信封，且 `messageId` 在事件发出前已持久化。
测试：`AguiServiceTest`、`ChatTurnServiceTest`、`CustomerServiceControllerTest`。
...
customer-work.memory:
  ...
  store-mode: jdbc          # provider=memory 时：jdbc（生产基线）| memory（开发/测试）
  consent-required: true    # prod 显式开启；未同意时不写入也不召回
...
- 隔离键由 `MemorySubjectResolver` 按“可信租户 + 主体类型/ID + Agent”构建并 SHA-256 去标识化；只有入口
  验签得到的 `USER` 主体跨自己的会话共享，其它入口按完整 `SESSION` 隔离。
- 终端用户通过 `/api/customer/user/privacy/**` 管理同意、查看、导出和删除；保留策略见 §6.34。
- 百炼/Mem0/ReMe 适配器可用于非生产环境。prod 的 `ProductionReadinessValidator` 在长期记忆开启时强制
  `provider=memory`、`store-mode=jdbc`、`consent-store-mode=jdbc`，因为外部 Provider 尚无统一的主体导出、
  擦除与可验证回执 SPI；不能绕过门禁后宣称合规删除。

测试：`LongTermMemoryTest`（记录/召回/租户隔离）、`LongTermMemoryProviderTest`（provider 选择）、
`MemorySubjectResolverTest` / `MemoryPrivacyGovernanceTest` / `UserPrivacyControllerTest`（主体、同意与隐私 API）。
...
### 6.9b 会话状态乐观并发与最终答复过滤（AgentScope 2.0.3）

```yaml
customer-work.agent:
  state-conflict-policy: OVERWRITE      # OVERWRITE(默认) / FAIL / APPEND_MERGE
  final-answer-filter-enabled: false    # 默认关闭，打开会让流式打字机效果消失
```

**状态乐观并发升级后默认就在生效**，不是可选项。2.0.3 给 `AgentStateStore` 加了版本化写入：
读状态带回版本号，写回走 `saveIfVersion` 做 CAS；`MysqlAgentStateStore#supportsVersioning()`
硬编码返回 `true`，因此客服端与后台两条链路都在走这条路径。本配置只决定 CAS 失败那一刻怎么办：

| 取值 | 行为 | 什么时候用 |
|---|---|---|
| `OVERWRITE` | 重读最新版本后覆盖写 | 默认。等价于升级前「最后写入者赢」的行为 |
| `FAIL` | 抛 `ConcurrentSessionModificationException` | 需要让调用方感知冲突并自行重试的集成 |
| `APPEND_MERGE` | 把本次新增的消息追加到最新版本之后 | 不希望丢消息、且能接受顺序由到达时刻决定 |

默认保持 `OVERWRITE` 的理由：客服会话已由 `SessionLock` 做串行化，真正并发写只出现在锁降级
（Redis 故障回落进程内）与多副本竞态这类少数场景，此时中断用户对话（`FAIL`）比覆盖一次状态更伤。

**`OVERWRITE` 会把冲突悄悄吸收掉**——不抛异常、不打日志。因此加了指标
`customerwork.agent.state.conflicts`（Prometheus 侧 `customerwork_agent_state_conflicts_total`，
标签 `owner` 区分链路），采集点在 `AgentResourceCloser` 这个唯一的 Agent 释放入口。
长期为 0 说明串行锁在守；突然抬头通常意味着 `SessionLock` 已保护性降级到进程内。

**升级前置**：CAS 依赖表上的 `version` 列，运行账号只有 DML 权限的部署必须先由 DBA 补列，
否则应用起不来。见部署手册第三节。

**最终答复过滤**抑制「产生了工具调用那一轮」的中间文本，用意是不让「我先查一下订单」这类
过渡语混进最终答复。**默认关闭**：它要等 `ModelCallEndEvent` 才把整轮文本一次性放出，
流式打字机效果因此完全消失，用户会盯着空白等几秒再看到整段答复；而客服场景里那句过渡语
恰恰是有价值的等待反馈。只在非流式集成（如渠道机器人只取最终文本）里才值得打开。

### 6.9c 多智能体编排拓扑（AgentScope 无关，能力差距 P1-15）

```yaml
customer-work.multi-agent:
  mode: fanout            # fanout 并行聚合 | sequential 串行细化
  experts:
    - name: OrderExpert
      sys-prompt: "你是订单与物流专家。..."
      tool-groups: [order]        # 目前只认 order / after_sales / knowledge，见下
      intents: [order]            # 认领哪些意图
      enabled: true
      order: 10                   # sequential 的流转顺序
  route-keywords:                 # 快车道：命中唯一意图即直路由，跳过 LLM 分诊
    order: [物流, 快递, 到哪了, 发货, 签收, 运单, 几天到]
```

改造前这三样东西全部硬编码：专家清单与提示词写死在 `buildSpecialists()`、意图到专家的映射
写死成一个 switch、快车道关键词写死成一张静态表。改一句提示词、调一个业务触发词、增删一个专家，
都要改代码重新发版。**默认值逐字等于改造前那三个专家**，不配置时行为完全不变。

**关键词为什么挂在意图上而不是专家上**：快车道的判定是「命中<b>唯一意图</b>」，
而一个专家可以认领多个意图（售后专家同时管 `refund` 与 `complaint`）。按专家分组的话，
「我要退款而且要投诉」会命中同一个专家、被当成唯一命中直路由，而原实现命中两类意图、
交给 LLM 分诊——那是一次不该夹带的行为变更。

**工具组目前只认三个**（`order` / `after_sales` / `knowledge`）：编排器只注入了这三个后端。
填了 `presale` / `member` / `complaint` 会记 `MAS-TOOL-GROUP-UNKNOWN` 并跳过，
**不静默忽略**——「配了不生效」比没有这个配置项更糟，而这种错配只表现为
「这个专家什么都查不到」。要支持全部七个组，得让编排器改用 `ToolRegistrar`。

**投票 / 辩论 / 黑板仍然不做**：客服场景的多专家协作诉求没有强到需要群体决策机制，
`/consult` 目前只有后台在用。

### 6.9d MCP 工具契约快照与漂移检测（能力差距 P1-9）

MCP 工具的定义住在**远端服务器**，随时可能变：加一个必填参数、改一个字段类型、下线一个工具。
项目对模型、提示词、路由都有完整的版本治理，此前唯独 MCP 没有任何基线——
`listTools` 的结果只拿去调试面板展示、不落库。

后台「AI 配置 → MCP 管理」每行新增**契约**入口：

| 操作 | 接口 | 权限 |
|---|---|---|
| 采集一次 | `POST /api/aiconfig/mcp/{id}/contract/capture` | `mcp:view` |
| 最新快照 | `GET /api/aiconfig/mcp/{id}/contract` | `mcp:view` |
| 历史 | `GET /api/aiconfig/mcp/{id}/contract/history?limit=20` | `mcp:view` |

**分级只有一条判据**：模型按旧契约发出的调用，在新契约下还能不能成功。

| 级别 | 包含 |
|---|---|
| `BREAKING` | 工具下线、新增必填字段、字段消失、字段类型改变、参数结构改变 |
| `COMPATIBLE` | 新增工具、新增可选字段、必填放宽、工具描述修改 |
| `NONE` | 指纹一致 |

三条约定：
① **基线就是上一条快照**，不设状态位——每次采集插一行（表 `ai_mcp_tool_contract`），
漂移只在发生的那次被记下，新快照自然成为下次的基线，像 git log；
② **检测到破坏性漂移不会停用这个 MCP**，只报告并记 `MCP-CONTRACT-DRIFT-BREAKING`。
上游演进是常态，为一次漂移把业务能力整个关掉代价远大于收益，何况基线本身可能已经过期；
③ **字段级 schema 片段只取 `type` 参与契约**，片段里的 description / examples / default 不看——
整段进来会让上游随手补一句字段说明都变成一次漂移事件。代价是字段说明改了这里看不见。
...
> **唯一权威持久化**：生产只注册 `TicketStore`，由 `ticket.store-mode=jdbc` 写入 `cw_ticket`；
> `HandoffService` 把旧 PENDING/CLAIMED/RESOLVED 合同投影到完整 Ticket 状态机，创建、接单、结案、
> 智能路由和 SLA 不再写 `cw_handoff_ticket`。V17 归并存量后，旧表只读归档。
...
> 测试：`HandoffServiceWiringTest`（生产只注入 TicketService）、`HumanHandoffToolsTest`（工具单写权威工单）、
> `CustomerWorkSchemaMigrationIntegrationTest`（V17 存量合并与镜像接管）。

**人机切换 SLA 升级引擎**：统一由 `TicketSlaScheduler` 扫描 `cw_ticket`。`WAITING_AGENT`（超
`sla-waiting-seconds`）与 `PROCESSING`（超 `sla-processing-seconds`）只告警，不自动接单/结案；
`WAITING_CONFIRM` 超时自动确认，`RESOLVED` 超时自动归档。`HandoffSlaScheduler` 只保留为旧表迁移回归夹具，
不再注册定时任务。
> 测试：`TicketSlaSchedulerTest` 覆盖告警、自动确认、自动关闭与单单失败隔离。
...
customer-work.model:
  ...
  cost-control:
    enabled: true                                               # 当前仅保留配置与孤立组件，主链未读取，不会自动拒绝请求
    max-tokens-per-minute: 100000                               # 不能替代 TenantQuotaGuard / SubjectQuotaGuard
    max-tokens-per-hour: 1000000
  tiered-routing:
...
customer-work.security:
  auth:
    ...
    key-id-header-name: X-API-Key-Id
    credentials:
      - key-id: partner-a
        key-hash: <原始 secret 的完整 SHA-256 小写摘要>
        tenant-id: tenant-a
        scopes: ["POST:/api/customer/**"]
        expires-at: 2027-01-01T00:00:00Z
        epoch: 2
        enabled: true
    minimum-epochs: { partner-a: 2 }
  rate-limit:
    enabled: true
...
```bash
curl -X POST localhost:8080/api/customer/chat \
  -H "X-API-Key-Id: partner-a" -H "X-API-Key: <原始 secret>" \
  -H "Content-Type: application/json" -d '{"message":"你好"}'
```
健康检查 / Actuator 免鉴权。结构化凭据支持 scope、expiry、停用和同 keyId 多 epoch 无中断轮换；提高
`minimum-epochs` 即撤销旧 epoch。`api-keys/tenant-keys` 仅为非生产兼容入口，prod 门禁拒绝明文配置。
限流、主体配额与审计只使用稳定 keyId，不把原始 secret 写入 Redis key 或日志。测试：
`ApiKeyAuthWebFilterTest`、`RateLimitWebFilterTest`、`ProductionReadinessValidatorTest`。
...
当前运行时过滤器会合并所有租户的启用词；在改为按租户分片前，写入、编辑、启停、删除与导入要求
`control_plane=1` 并继续叠加原 `sensitive-word:add/edit/delete` 权限点，普通租户保留只读与导出。
...
后台 `/aiconfig/mcp` 的数据出口和权限刻意分层：

- `GET /api/aiconfig/mcp` 只返回非敏感摘要，`config` 内容为空；`GET /{id}` 是编辑详情，要求 `mcp:edit`。
- 编辑详情保留 JSON 结构，但敏感字段及 `headers` 下所有叶子值统一替换为
  `__MCP_SECRET_REDACTED__`；更新时仅在连接目标未变化时，服务端才按相同 JSON 路径把占位符合并为旧值。
  远程传输以 `type + scheme/host/effective port/path` 标识目标，stdio 以 `type + command/args/working directory`
  标识执行目标；目标变化或新建时必须重新提供全部凭据。
- 远程 MCP URL 仅接受 http/https，并禁止 userInfo、query、fragment；凭据必须从 URL 移出并放在 `headers`。
  保存时服务端把 headers/token/password 等敏感叶子提取为路径键控的 SecretRef 材料，config 只留不可执行占位符；存量非法 URL 的编辑详情按无效配置 fail-closed；最终客户端构建和运行时发布
  也会再次校验，发布不会静默丢掉该工具。配置版本详情会把历史危险/畸形 URL 整段脱敏。
- `test-connectivity` 和 `debug/tools` 仅探测可达性/工具清单，使用 `mcp:view`；`debug/call` 可能改变下游数据，
  必须具备 `mcp:edit` 并写操作审计。
- 执行边界才按 tenant + refId 解出材料；Nacos 中只发布 AES-GCM `headersCipher`。生产启动会拒绝仍含内联凭据的
  启用项，停用存量可通过编辑后重新启用完成迁移；页面同时展示 SecretRef 状态、版本与到期时间。

测试：`McpToolkitConfigurerTest`、`HigressToolkitConfigurerTest`（开关与装配判定，不连真实服务），以及
`McpServiceTest` / `McpControllerSecurityContractTest`（摘要、详情权限、脱敏占位符合并与调试权限）。
...
java -jar customer-channel/target/customer-channel-2.4.0.jar     # 端口 8081（版本号随父 POM）
```
| 入口 | 能力 |
|---|---|
...
java -jar customer-admin-server/target/customer-admin-server-*.jar --spring.profiles.active=dev

# 前端：端口 5174（vite.config.ts 固定），开发期把 /api 代理到后端 8082，见 customer-admin-web/vite.config.ts
cd customer-admin-web && npm ci && npm run dev
```
...
| `GET /api/auth/register-options` | 登录页要渲染成什么样：是否开放注册、发码那一步要不要图形码、发码冷却秒数、能否自助找回密码。邮箱与邮箱验证码不在其中——它们恒为必需，前端无条件渲染 |
| `POST /api/auth/email-code` | 向注册邮箱发一封 6 位验证码；先查邮箱占用再发信，图形码在这一步校验（发信是唯一会向站外第三方产生副作用的匿名操作）。四道限流见 `EmailVerificationService` |
| `POST /api/auth/register` | 登录页公开自助注册；**必须填邮箱并填回收到的验证码**（无开关可关），仅创建 `default` 租户的 `PENDING` 本地账号，不自动授予角色或菜单，`email_verified` 落 1 |
| `POST /api/auth/password-reset/email-code` · `POST /api/auth/password-reset` | 登录页「忘记密码?」；用户名与注册邮箱必须同时匹配同一个账号，但对不上时的响应与「邮箱未注册」完全一致（含扣掉发信额度）。发码那一步**无条件**校验图形验证码，重置那一步只认邮箱验证码。能力跟随邮件是否可用，无独立开关。成功后递增 `auth_epoch` 撤销既有会话、清零登录失败锁定、`email_verified` 置 1。详见《部署手册》§8.1 |
...
| `PUT /api/aiconfig/model/{id}/health-override` | 以 `AUTO/FORCE_HEALTHY/FORCE_UNHEALTHY + reason + expiresAt` 管理人工健康覆盖；要求 `model:health-override`，并持久化操作者与事件 |
| `PUT /api/aiconfig/model/{id}/credential` | 轮换凭据并推进 SecretRef version；旧健康/认证事实随版本变化失效 |
| `/api/aiconfig/model-routing-policies/**` | 不可变路由版本、冲突校验、激活与 dry-run；实际调用由 runtime 同一策略选路 |
| `/api/aiconfig/model-experiments/**` | 双臂实验创建/启动/停止、事件与真实调用指标；启动前分别以同一已审核数据集版本离线评测 control/treatment；desired/effective state 分列 |
| `/api/eval/datasets/**` | 评测工作集 CRUD、导入导出、命名版本、审核与版本 diff；版本冻结不可变快照和内容 hash |
| `/api/aiconfig/runtime-publish/gate/**` | Eval release gate 策略、任务判定、重评与绑定候选 hash 的审计豁免 |
| `GET /api/agent-call-stats/{id}/replay-manifest` | 安全重放清单，冻结模型参数摘要、RAG/工具摘要、已录输出与运行时/制品版本；读取本身不执行 |
| `POST /api/agent-call-stats/{id}/replay` | 默认 MOCK 重放并输出答案/制品差异；不存在 LIVE 模式，外部调用数恒为 0；DRY_RUN 仅允许显式配置的隔离部署 |
| `/api/slo/**` · `/api/business-outcomes/**` | SLO/error budget 策略、周期/手工评估、告警确认/事件；业务结果汇总与会话下钻，见 §6.34 |
| `/api/improvement-cases/**` | KnowledgeGap/badcase 共用的责任认领、回归用例、精确制品、复评、可靠发布及线上效果闭环，见 §6.31、§6.34 |
| `/api/aiconfig/mcp` | MCP 管理：分页不返回 config；编辑详情要求 `mcp:edit` 并返回脱敏占位结构；`test-connectivity`/`debug/tools` 用 `mcp:view`，`debug/call` 用 `mcp:edit` 且审计；删除前校验智能体引用 |
| `/api/aiconfig/skill` | Skill 管理 CRUD；每次创建/编辑生成不可变版本，`/{id}/versions` 查询版本；编辑后自动失效引用它的智能体运行时缓存，`/{id}/download` 导出技能包 zip（`skill:export`） |
| `/api/aiconfig/knowledge-base/{id}/versions` | 查询知识库不可变版本、checkpoint、快照 hash、文档数和质量事实 |
| `/api/aiconfig/knowledge-base/{id}/sources/**` | KnowledgeOps 文档源 CRUD、PUSH 增量/删除/全量同步、同步运行、文档 lineage；同步要求独立权限 `knowledge-base:source-sync` |
...
**KnowledgeOps 同步契约**：当前可执行来源类型为 `PUSH`。每批请求用 `requestId` 保证幂等，
`expectedCheckpoint` 必须等于来源当前 checkpoint，成功后才原子推进到新的 `checkpoint`。增量批次的
`documents[].operation` 支持 `UPSERT/DELETE`；`fullSnapshot=true` 时只允许 UPSERT，未出现在本批快照的旧文档
会自动生成删除修订。同步失败、checkpoint 冲突或质量分低于来源门槛都不会发布版本。质量分取正文去重率与
`expectedDocumentCount` 覆盖率的较小值。文档 ACL 支持 `PUBLIC/RESTRICTED`，受限文档按可信主体类型、主体 ID、
渠道中实际配置的维度共同匹配，未知/损坏 ACL 默认拒绝。管理端“知识运营”抽屉可查看来源新鲜度、运行事实、
lineage 与不可变版本。

```json
{
  "requestId": "erp-kb-20260824-001",
  "expectedCheckpoint": "offset-41",
  "checkpoint": "offset-42",
  "fullSnapshot": false,
  "expectedDocumentCount": 200,
  "documents": [{
    "operation": "UPSERT",
    "externalId": "refund-policy-42",
    "sourceVersion": "v7",
    "title": "退款政策",
    "sourceUri": "https://kb.example.test/refund-policy-42",
    "content": "……",
    "acl": {
      "mode": "RESTRICTED",
      "allowedSubjectTypes": ["USER"],
      "allowedSubjectIds": ["42"],
      "allowedChannels": ["user-http"]
    }
  }]
}
```

模型端点出网规则由 `ModelEndpointPolicy` 收口：`baseUrl` 仅接受 http/https 且不得含 userinfo、query、fragment，
保存时解析全部地址；`admin.model.egress.allowed-hosts` 为空时允许公网、拒绝 RFC1918/IPv6 ULA 等内部地址。
该列表一旦非空就变成严格 host 白名单，支持精确值和 `*.example.com`，未列出的公网 host 也会被拒绝；名单只会
放宽 RFC1918/ULA，环回、链路本地（含 `169.254.169.254`）、未指定和组播地址始终拒绝。混合公网/私网模型的部署必须列全两类 host。

连通性/健康探测及认证的连通性采样使用受控 OkHttp：每次连接重新解析并校验 DNS，把同一次解析结果直接用于建连，
且禁用 http/https 自动重定向。已有 SecretRef 或旧密文凭据的模型修改 `baseUrl` 时必须同时重提凭据；没有现有
凭据的存量模型可以只修正端点。边界是认证的流式/工具调用/结构化输出检查与实际推理会经
...
**跨会话长期记忆**（`capabilities` 含 `memory`，V32 起）：V86 起权威快照携带单调递增 `version`；
每个 workspace 记录水合时版本，回写通过 CAS 完成。并发 Pod 基于旧版本写入时保留已提交的权威内容并记录错误，
不会静默覆盖。磁盘后端同样使用文件锁、版本侧车与原子替换保持同一语义。Harness 分层记忆（`MemoryConfig`，
对话事实自动 flush 到 `workspace/MEMORY.md` 并定期 consolidation），按 agentCode 隔离、全部会话共享。workspace 文件只是
...
`ModelConfigServiceTest`（模型资产/部署与 SecretRef 协作、脱敏回显、默认模型互斥事务）、
`SecretRefServiceTest`（版本优先、历史密文兼容、租户边界、轮换与无明文返回）、
`ModelHealthServiceTest` / `ModelCertificationServiceTest` / `ModelRoutingPolicyServiceTest`（健康、认证与路由约束）、
`ModelEndpointPolicyTest`（公网默认、严格 host 白名单、私网放宽与永久拒绝地址、DNS rebinding 边界）、
`ChatModelProberTest`（四厂商探测协议、固定解析、禁重定向、响应结构与硬超时）、
`AdminModelFactoryTest`（探测委托/结果翻译与 `buildModel` provider 分支）、
...
- **历史对话列表**：新增只读端点 `GET /workspace/{agentCode}/chat/sessions`（枚举
  ...
  一起，不做类型区分）。
- **视觉细节**：favicon 使用蓝绿配色的 CW 对话气泡品牌图标（多尺寸 ICO，保留 PNG 资源）；`MenuTree.vue` 无 `icon` 的菜单节点
...
- **鉴权体系**：用户 JWT（env `CW_USER_JWT_SECRET`）/ 坐席 HMAC 凭证（`AgentAccessCredential`，env
  `CW_AGENT_WS_SECRET`，8080 与 8082 共享）；开发有默认值、**生产必须覆盖**。JWT 携带账户
  `session_epoch`，HTTP、WebSocket 与主体配额入口均按租户回查；H5 的“退出所有设备”调用
  `POST /api/customer/auth/revoke-sessions` 原子推进 epoch，使全部旧令牌立即失效。
- **统一人机切换域**：`HumanHandoffTools.transferToHuman` 与真实用户会话入口都调用 `TicketService`；
  旧 `/handoffs` 接口仅做三态投影，V17 将 `cw_handoff_ticket` 存量归并后转为只读归档。
...
多副本部署下，Admin 对话以 `tenant + agent + session` 作为 Redisson permit 锁键，锁服务不可用时失败关闭，
不退化为各 Pod 独立的本地锁。Plan/HITL 的 `PENDING` 与终态保存在 `ai_plan_confirmation`，任意 Pod
均可执行一次性状态转换，原始流所在 Pod 观察共享终态后恢复挂起调用。等待/租约可通过
`admin.agent-runtime.session-lock-wait-seconds` 与 `session-lock-lease-seconds` 配置。
...
（跨租户定位 + 按任务租户还原上下文）、登录页轮播图 `login_carousel_image` 归入全局级忽略清单。
...
存量数据、共享配置与后台用户统一归入 `default`；遗留特殊租户由 V63 / V7 归并，V8 补齐客服端遗留分区键。
...
| 后台管理 | 登录用户的 `sys_user.tenant_id` + 角色 | `control_plane=1` 且具备对应权限点时可切换目标租户视角 |

**后台租户管理**：`系统管理 → 租户管理`（同时要求控制面角色与 `tenant:view/add/edit/delete` 权限点）。
新建租户会自动初始化一个 `tenant_admin` 角色，并通过 `ControlPlanePermissions` 排除控制面专属权限；
角色编辑接口使用同一策略防止重新授予，V63 同步清理存量普通角色。
以下生命周期操作针对非 `default` 业务租户；`default` 是不允许冻结/退租且无需访问快照的保留租户。
冻结、到期变更、手工撤销与退租会递增 `accessEpoch`，并在同一事务写入租户访问快照投递任务；
当前控制面实例在提交后撤销该租户后台登录态。8080 的新请求与 WebSocket 握手按内存快照 fail-closed，
应用冻结/退租快照后还会主动断开该租户已有的用户/坐席 WebSocket。退租同时停用渠道绑定，保留租户主记录与
业务数据用于审计和后续数据交付，不等于级联物理删除。跨运行时生效仍取决于 Nacos 投递、监听与主动回读，
存在可观测的快照传播延迟；`GET /api/tenant/{id}/access-delivery` 返回最近一次访问快照的交付步骤与状态。
`control_plane` 不开放给普通角色编辑接口；控制面角色的分配、移除、恢复、编辑与删除同样要求调用者已有控制面身份。
...
**已知限制**：客服端的字典 / 敏感词 / 限流规则三张配置表，新租户开通时不会自动复制 `default` 内置值
（admin 与客服端库之间没有直连通道），当前需要为新租户单独维护；共享词库批量下发属于运维能力，留待后续批次。

### 6.28 水平扩展：限流 / 租户与主体配额 / 会话锁的跨实例共享（默认关闭）
...
customer-work:
  distributed:
    counter-mode: redis        # 限流 + 租户配额 + 主体配额共用的窗口计数
...
admin:
  billing:
    ...
    settlement:
      zone-id: Asia/Shanghai  # 自然日账单时区
      max-reconciliation-days: 31
```
...
**结算与账单口径**：每个 MODEL 分段在调用结束时，以该段冻结的 `priceId/currency/inputPrice/outputPrice`
和真实 usage 结算金额，金额保留 14 位小数；调用主记录汇总这些不可变事实。缺价格、缺 usage、负数 usage
分别进入 `UNPRICED/USAGE_MISSING/USAGE_INVALID`，调用进入 `PARTIAL/UNAVAILABLE`，不得写成 0。
调价请新增一条生效记录而不是改旧记录，既有调用金额不会随价格表变化。

T+1 归集以 `zone-id` 的自然日为边界，先锁定全局日期行并冻结客服库最大 MODEL callId，再从调用分段的
已结算金额整日重建 `cw_tenant_usage_daily`，写后回读比较精确事实；因此多副本不会并发覆盖，失败也不会
留下“部分成功”的账单。`GET /api/billing/reconciliation` 按租户、日期、币种输出
`MATCHED/INCOMPLETE/STALE/MISMATCH`，最多查询 `max-reconciliation-days` 天。金额不能跨币种相加。

**预算治理接口**：`GET /api/billing/forecast` 按当前账期已过天数做金额线性外推；
`GET /api/billing/alerts` 查询 `BUDGET_WARNING/BUDGET_EXCEEDED/FORECAST_EXCEEDED` 告警，
`POST /api/billing/alerts/{id}/ack` 幂等确认；`GET /api/billing/export` 导出带 BOM 的 CSV，并对公式前缀转义。
告警按租户、账期与类型等业务键去重，落库后尝试向具备账单查看权限的用户投递站内消息；消息失败不回滚
唯一告警事实。预测没有季节性、节假日、增长趋势或置信区间，只能作为预算早期信号，不能当作财务承诺。
...
ACK 使用独立的 `X-Runtime-Config-Ack-Token`；admin 通过
`admin.runtime-publish.ack-identities=tenantId|instanceId|token,...` 把每个至少 32 字节的 token 固定绑定到
租户和实例，再核对正文 instanceId/revision/contentHash。通用开放 API token 不具备 ACK 权限，单个凭据也不能
伪造多个 instanceId 累加阈值。
渠道绑定页的“重新发布”只负责把任务可靠入队并返回 taskId，列表状态列持续展示
`PENDING/PROCESSING/PUBLISHED/PARTIAL/APPLIED/FAILED`；失败原因通过标签提示查看。

8080 消费端使用发布端共享的 `RuntimeConfigContentHasher` 对解析后的业务正文重算 SHA-256；计算时排除
`publishedAt`、`revision`、`contentHash` 三个投递元数据。声明 hash 缺失、不是无空白的 64 位十六进制文本，
或与重算结果不一致时，实例回传 `REJECTED` 并保留旧配置，不清语义缓存也不进入 applier。
hash 校验后再按 keyId 验 HMAC-SHA256，签名覆盖 revision/contentHash/publishedAt/schemaVersion；未知 key、缺签名
或篡改在任何密文解密前拒绝。签名 key 支持当前 key + 旧 key 映射滚动轮换，Nacos ACL/TLS 仍作为纵深防御。

运行时配置的 `contentHash` 变化时，8080 会先阻断当前租户缓存读写并清理旧代际，再应用新配置。聊天请求在入口
捕获当前 generation，查缓存与最终写入复用同一代际；切换前已经开始的旧请求完成后，会因 generation 不匹配而
拒绝回写。清理失败不进入配置应用；配置应用失败则回滚旧 generation，继续保留旧配置。

新任务在入队时冻结该租户 ACK identities 的 instanceId 集合；只有集合内全部实例都回执 APPLIED 才完成，集合外
回执拒绝。`minimum-ack-count` 仅用于没有 `ack_targets_json` 的历史任务兼容聚合。

回滚和灰度不再由单人直接执行：原接口创建 `ai_governed_change_request`，不同账号持
`governance:approve` 复核后才调用类型化执行器。SUBMITTED/APPROVED/EXECUTED/FAILED 等事件同步追加到按请求串联的
SHA-256 审计链，最短留存默认 3650 天；宕机遗留 EXECUTING 超时收敛 FAILED，可重新发起但不会自动重复执行。
...
- 多租户或显式配置 `customer-work.nacos.tenant-code` 时，客服端只读、只监听对应租户 dataId；缺失或删除时
  保留最后一份安全配置，不回落、也不响应主 dataId；多租户未配置 `tenant-code` 会订阅失败并持续重试；
- 只有 `customer-work.tenant.enabled=false` 且 `tenant-code` 为空的单租户部署读取主 dataId。灰度撤销必须通过
  可靠发布任务把权威配置重发到同一租户 dataId，不能靠删除租户 key 回落主配置。

客服端不猜测"灰度"语义：租户 dataId 是租户运行时配置的唯一权威来源，主 dataId 与租户域严格隔离。
...
| badcase 回流 | `customer-work.badcase` | `cw_badcase` | `/ops/badcase`，`badcase:view` / `badcase:adopt`；闭环管理另需 `improvement:manage` |
...
| 知识盲区 | `customer-work.knowledge-gap` | `cw_knowledge_gap` | `/ops/knowledge-gap`，`knowledge-gap:view`；闭环管理另需 `improvement:manage` |
...
GET    /api/eval/datasets/{type}/cases      查询 INTENT/QUALITY 工作集
POST   /api/eval/datasets/{type}/cases      新增用例（PUT/DELETE 更新或删除）
POST   /api/eval/datasets/{type}/import     批量导入（GET .../export 导出）
POST   /api/eval/datasets/{type}/versions   冻结命名版本
POST   /api/eval/datasets/versions/{id}/review  审核版本
GET    /api/eval/datasets/versions/diff     比较两个不可变版本
...
GET    /api/improvement-cases/source/{type}/{sourceKey}  按原始信号查询闭环
POST   /api/improvement-cases/source/{type}/{sourceKey}/triage  认领并设置 SLA
POST   /api/improvement-cases/{id}/eval-case             创建目标回归用例
POST   /api/improvement-cases/{id}/artifact              绑定 Agent 当前精确候选
POST   /api/improvement-cases/{id}/reevaluate            复评（需 eval:run）
POST   /api/improvement-cases/{id}/publish               可靠发布（需 agent:edit）
POST   /api/improvement-cases/{id}/refresh               催办自动状态同步
...
评测工作集支持 CRUD、导入导出；创建命名版本时固化不可变用例序列与内容 hash，并在独立审核后才可供实验使用。
每次评测运行事实绑定评测集、模型、提示词、Agent、知识库、工具、Judge 和 rubric 九类版本。运行时发布门禁支持指标阈值、相对基线回归、关键用例与 Judge 故障策略；
`GET /api/aiconfig/runtime-publish/gate/tasks/{taskId}` 查看判定，失败任务可重评，紧急豁免必须绑定 taskId 与
候选内容 hash，并记录操作人、原因与原判定。模型实验创建时绑定已审核 QUALITY 数据集版本；ACTIVATE 在
无主库长事务的阶段分别评测 control/treatment，两臂都通过后才进入短事务激活，再叠加模型认证与线上错误率/P95 护栏。
...
- **语义缓存框架默认关闭，开启后也只缓存通用问答**：无差别缓存会把 A 用户的订单信息返回给 B。三道闸门收口在
  ...
  6 位以上连续数字即跳过）+ `scope_id`/`tenant_id` 双层租户分区。往 `cacheable-intents` 加意图前，先回答
...
- **语义缓存以可信租户作为硬隔离边界**：持久层每次读写都带 `tenant_id`，内存实现也按 `TenantContext`
  分区。`scope_id` 是 `TenantResolver` 解析的数据 scope；生产请求存在可信上下文时优先取当前租户，绝不允许
  客户端伪造 sessionId 前缀切换租户。无上下文的前缀解析只是历史兼容。缓存仍仅允许通用意图并过滤长数字标识，
  不能把这套通用问答缓存用于订单、用户画像等个性化答案。`GET /api/ops/semantic-cache/scopes` 只列当前租户
  实际分区并限额（默认 100）。
- **语义缓存同时绑定运行配置 generation**：每个请求在入口捕获当前 `contentHash`，查找与写入只访问同一代际。
  热配置切换先暂停缓存读写、清空旧代际，再按配置应用结果提交新 generation 或回滚旧 generation；因此旧请求
  即使跨过切换点完成，也不能把旧模型答案写进新配置缓存。
...
- **运营分区（`scope_id`）取可信租户**（`OpsScopeResolver`）：CSAT 与知识盲区是租户级运营指标，不能按
  sessionId 前缀拆成每人一张报表。长期记忆另由 `MemorySubjectResolver` 按“租户 + 已验签主体 + Agent”建
  去标识化主体键；语义缓存使用 `TenantResolver` 的数据 scope，并由 `tenant_id` 再做持久层硬隔离。
  三者业务粒度不同，不要再次合并成一个解析规则。
- **知识未命中文案是接口契约**（`KnowledgeBackend.NO_HIT_REPLY` + `isMiss`）：盲区埋点据此判定，
  改文案会让统计静默失效。太短的问题（"嗯""在吗"）不计入，否则会淹没真正的盲区。
- **改进闭环不以“已发版”冒充“已有效”**：复评只接受与冻结候选一致的完整运行，目标用例必须通过、不能新增回归，
  发布前再次校验候选未漂移。只有可靠发布任务达到全目标 `APPLIED` 才开始观察，并按精确 runtime revision 统计曝光、
  按归一化问题哈希统计信号增量；复发即 `INEFFECTIVE`，观察期满且曝光达标才 `VERIFIED`，低流量只能
  `INCONCLUSIVE`。跨 Pod 自动处理以 `ai_agent_improvement_case` 数据库租约协调。
...
**三类主体，一套机制**：登录用户按 `userId`、匿名调用方按来源 IP、服务端接入方按稳定 API Key
`keyId`（`QuotaSubject`）。兼容旧 Key 只生成 SHA-256 前 16 位指纹；任何路径都不会把原始 secret
写入 Redis 计数键、命中表或日志。
...
（free / vip / svip / anonymous / api-key）；租户没配某一档时回落 `default` 租户的同名档，再回落
...
#### 6.32.1 后台登录用户的额度（`admin.subject-quota`，示例 admin 默认开启）
...
| 取值 | 含义 | 默认落在谁身上 |
|---|---|---|
| `ALL` | 不受当前租户内的用户维度限制 | `super_admin`、`operator` |
| `TENANT` | 本租户内所有成员的数据都可见 | `tenant_admin` |
| `SELF` | 只看自己创建的个人产出物 | 其余一切角色，含新建角色的默认值 |

多角色取最宽；`ALL` 额外要求当前用户具备 `control_plane=1`（否则降级为 `TENANT`），前端对无控制面能力的用户不显示该选项、
后端 `RoleService` 保存时再拦一次。跨租户操作还要叠加接口权限点。范围每次请求实时解析，**改完立即生效，无需重新登录**。
...
`ai_chat_attachment.create_by`、`ai_chat_session_owner` 会话归属表）+ V63（归并遗留租户并新增 `control_plane`）。
V7 / V8 / V63 部署需维护窗停写并先备份，执行顺序为 customer-work V7、V8 → admin V63 → 启动新代码。
迁移 SQL 只匹配精确旧租户值，真实业务租户保持不变。启动前还必须把 `NACOS_TENANT_CODE` 改成 `default`、
将租户专属 Nacos dataId 重发到 `-tenant-default`，并轮换所有携带旧租户值的登录态和机器凭据；
Redis 配额旧计数与 MinIO 历史工作区由过渡兼容逻辑承接，完整核验和移除条件见 `docs/多租户架构设计.md` §5。

### 6.34 企业级 AI Agent 治理与运维闭环

本节只列当前接口与运行语义；上线前置条件、故障处置和剩余边界见
[企业级 AI 智能体能力与运维边界](企业级AI智能体能力与运维边界.md)。

| 治理域 | 当前能力与入口 | 必须保留的边界 |
|---|---|---|
| 租户撤权 / 退租 | 用户 `authEpoch`、租户 `accessEpoch`、后台登录态撤销、渠道停用、访问快照可靠投递与 `/api/tenant/{id}/access-delivery` 状态；8080 对新请求/WS 握手 fail-closed，冻结/退租或 ACTIVE epoch 递增都会主动断开旧 WS，新 epoch 可重连 | 跨运行时仍有 Nacos 投递、监听和主动回读延迟；退租保留业务数据，不等于物理删除或已生成归档包 |
| 主体记忆隐私 | `/api/customer/user/privacy/memory-consent` 查询/更新同意，`/memory` 查看或删除，`/memory/export` 导出；未同意时不写入也不召回，保留任务分批清理 | 只有已验签 `USER` 主体跨会话共享；生产门禁强制 `provider=memory` 且记忆/同意均为 JDBC；外部 Provider 待补导出/擦除回执 SPI |
| FinOps | `/api/billing/forecast`、`/alerts`、`/alerts/{id}/ack`、`/export`、`/reconciliation`；token 实时配额与逐调用冻结价结算、T+1 精确账单并存 | 金额预测是简单线性外推，无季节性和置信区间；缺价、缺用量或跨币种时显式报告不完整，不作伪金额 |
| EvalOps / release gate | 工作集 CRUD/导入导出、命名版本、独立审核与 diff；不可变评测集快照与内容 hash、九维版本绑定、阈值/回归/关键用例/Judge 故障门禁、重评和绑定候选 hash 的审计豁免；实验 ACTIVATE 前分别离线评测 control/treatment | 远程 Judge/模型评测有真实费用；任一臂失败时实验保持未激活，但两臂事实均保留供审计 |
| Agent 改进闭环 | KnowledgeGap/badcase 页面共用 `/api/improvement-cases/**`：owner/SLA 认领、目标回归用例、当前 Agent 精确候选、复评、可靠发布任务与 revision 线上效果；多副本 Worker 用数据库租约跟踪 | 复评、发布分别额外要求 `eval:run`、`agent:edit`；观察采用精确问题哈希而非语义聚类，曝光不足只给 `INCONCLUSIVE`；参数见 `admin.improvement.automation.*` |
| ModelOps | `/aiconfig/model` 管理资产与部署；SecretRef、端点保存期出网校验、带连续失败/恢复阈值和冷却的健康快照/事件、限时人工 override、上线认证、引用影响分析；不可变路由版本、校验、dry-run 与动态健康 overlay 真实选路 | 当前 SecretRef 仅实现 `LOCAL_AES`；固定 DNS/禁重定向只覆盖连通性/健康探测及认证的连通性采样，AgentScope 厂商 SDK 的认证能力检查与实际推理 HTTP/DNS 仍需后续收口 |
| MCP 治理 | 分页不回 config；敏感叶子进入 SecretRef，详情只回脱敏占位符与版本/到期元数据；执行边界局部解析，Nacos 仅有 `headersCipher` | 当前 SecretRef provider 为 `LOCAL_AES`；外接 KMS 可沿 provider SPI 扩展，生产仍需数据库/Nacos ACL 与 TLS |
| Trace / Replay | 调用日志冻结 traceId、runtime revision/hash、最终模型参数白名单与输入 hash、RAG 引用摘要、工具输入形状/结果 hash、模型/提示词/Agent/知识/工具版本和实验曝光；读取清单或以 `POST /api/agent-call-stats/{id}/replay` 输出差异 | 默认 MOCK；不存在 LIVE，模型/RAG/工具外部调用数恒为 0；DRY_RUN 还需 `admin.agent-replay.dry-run-enabled=true` 且 `environment=isolated`，不持久化凭据、RAG 正文或工具原始结果 |
| 在线实验 | 不可变双臂、稳定分桶、曝光归因、调用日志指标、最小样本、错误率/P95 护栏、到期/自动停止；desired state 与 effective state 分列 | 发布任务冻结 ACK 目标并要求全集合 APPLIED；扩缩容后的新实例由后续 revision 纳入，不反写旧任务目标集合 |
| SLO / error budget | `/api/slo/policies` 配置，数据库租约周期评估或 `POST /api/slo/policies/{id}/evaluate` 手工评估；`/api/slo/alerts` 查询，`/{id}/ack` 确认，`/{id}/events` 查看 OPENED/ACKED/RESOLVED；状态事件与可靠通知任务原子提交 | `NO_DATA`/`INSUFFICIENT_DATA` 不打开也不恢复告警；只有达到最低样本且健康才 RESOLVED。通知投递给拥有 `slo:view` 的启用用户，失败无限退避重试；参数见 `admin.slo.automation.*` |
| 业务结果—成本 | `/api/business-outcomes/summary` 与 `/sessions` 汇总真实调用会话的技术成功、自动解决代理、转人工、token/金额完整性和 CSAT，并在完整单币种时给出会话成本与单次自动解决代理成本 | “自动解决”仍是无转人工且技术成功的代理口径；缺价、缺用量或混合币种时返回 availability，不按调用次数或 token 伪分摊 |
...
| `agent` | max-iters(10), meta-tool-enabled(false), **state-conflict-policy(OVERWRITE)**, **final-answer-filter-enabled(false)**（后两项为 AgentScope 2.0.3 能力采纳，见 §6.9b） |
| `memory` | long-term-enabled(true), provider(memory), store-mode(jdbc), consent-required(false；prod 显式 true), consent-store-mode(jdbc), retention-days(180), withdrawn-consent-retention-days(2555), retention-cleanup-{enabled(true),interval-ms(3600000),batch-size(1000)}, recall-scan-limit(500), tenant-delimiter(":"，仅无可信上下文的历史兼容), retrieve-top-k(5), bailian.*/mem0.*/reme.*；prod 启用长期记忆时强制 provider=memory 且两个 store-mode 均为 jdbc，外部适配器待导出/擦除回执 SPI；见 §6.34 |
...
| `admin.registration`（admin 自身配置）| self-service-enabled(true), captcha-enabled(false), trust-forwarded-header(false), trusted-proxy-cidrs([]), rate-limit{max-attempts(5), window-seconds(3600)}, captcha{length(4), ttl-seconds(180), width(120), height(40), max-issue-per-window(30)}, login-lock{enabled(false), max-failures(5), window-seconds(900)}, email-verification{code-length(6), ttl-seconds(600), max-attempts(5), resend-cooldown-seconds(60), max-send-per-email-per-day(10), max-send-per-ip-per-window(10)}。**邮箱必填与邮箱验证码没有开关**：任何部署形态下注册都必须填邮箱并填回收到的验证码（`RegistrationGuard#admit` 无条件核验），代价是自助注册开着时 SMTP 必须可用，prod profile 启动时即校验。**captcha-enabled / login-lock.enabled 在对外开放实例上被强制为 true，配 false 不生效**。转发头默认不信任；开启时可信代理 CIDR 不能为空，且仅从命中网段的直接连接方接收 XFF，再从右向左剥离可信代理链。图形码只在「发码」那一步校验，注册那一步只认邮箱验证码。**找回密码复用本组的全部限流参数与验证码参数**（发信额度跨用途共用一份，验证码本身按用途分键存储），但它的图形码无条件要求，不看 captcha-enabled |
| `admin.login-captcha`（admin 自身配置）| challenge-ttl-seconds(120), proof-ttl-seconds(120), max-issue-per-window(30), max-verify-per-window(10), max-proof-consume-per-window(120), rate-limit-window-seconds(120), max-in-memory-entries(10000), max-user-agent-length(512)。所有配置必须为正数，rate-limit-window-seconds 不得小于 challenge-ttl-seconds；独立于注册图形验证码且无关闭开关。签发、拼图落点/轨迹校验与 proof 消费分别按可信来源 IP 限流，三者共用限流窗口时长；验证预算默认每 120 秒 10 次，允许正常重复登录和拖动重试，在读取 challenge 前原子占用，成功、失败、过期或指纹不匹配都不返还，换 User-Agent 不会获得新预算；画布尺寸、轨迹点数、归一化坐标和时长是固定接口协议，不开放运行期调参；随机目标只存服务端，challenge/proof 都绑定 IP+UA 并只允许成功消费一次 |
...
| `security` | auth.{enabled(false),header-name(X-API-Key),key-id-header-name(X-API-Key-Id),credentials[{key-id,key-hash,tenant-id,scopes,expires-at,epoch,enabled}],minimum-epochs{},api-keys[]/tenant-keys{}(仅兼容，prod 禁止)}, rate-limit.{enabled(false),requests-per-minute(120),algorithm(fixed-window),window-seconds(60),rule-enabled(false),store-mode(memory),refresh-enabled(true),refresh-interval-ms(60000)} |
...
| `multi-agent` | enabled(true), mode(fanout), max-iters(6), max-concurrency(8), timeout-seconds(60), routing-enabled(true), fast-route-enabled(true), reduce-enabled(true), **experts（专家拓扑：名称/提示词/工具组/意图/启用/顺序）**, **route-keywords（意图 → 快车道触发词）**（后两项见 §6.9c） |
...
| `nacos`（含 B4 灰度）| tenant-code('')：多租户或租户专属实例必填，只消费对应租户 dataId；缺失/删除保留最后安全配置且不回落主 dataId，见 §6.30 |
| `nacos` | enabled(false), server-addr(localhost:8848), namespace, group(DEFAULT_GROUP), prompt-data-id, username, password, timeout-ms(3000), runtime-config-{enabled(false),data-id,ack-url,ack-token,instance-id,subscribe-retry-ms(10000)}, tenant-code(''), tenant-access-{enabled(false；prod 显式 true),data-id(customer-work-tenant-access),refresh-interval-ms(5000),max-staleness-ms(30000)}；见 §6.30、§6.34 |
| `admin.runtime-publish`（非 `customer-work.*` 前缀） | scan-interval-ms(5000), lease-ms(60000), batch-size(20), max-attempts(8), base-backoff-ms(5000), minimum-ack-count(1，历史兼容), ack-identities([]), signing.{enabled(false),key-id,secret}, nacos.{enabled(false),server-addr,namespace,group,data-id,username,password,timeout-ms}；新任务冻结 identities 并要求全集合 ACK，见 §6.30、§6.34 |
| `admin.improvement.automation`（非 `customer-work.*` 前缀） | enabled(true), scan-interval-ms(30000), lease-ms(120000), batch-size(50), observation-window-ms(86400000), min-exposure-calls(20), max-recurrence-signals(0), max-backoff-ms(300000)；只在发布任务 APPLIED 后观察精确 revision，见 §6.31、§6.34 |
| `admin.governance`（非 `customer-work.*` 前缀） | approval-expiry-hours(24), execution-timeout-seconds(600), audit-retention-days(3650), expiry-scan-interval-ms(60000)；生产最短留存不得少于 365 天 |
| `admin.model.egress`（非 `customer-work.*` 前缀） | allowed-hosts([])，环境变量 `ADMIN_MODEL_EGRESS_ALLOWED_HOSTS` 以逗号分隔，支持精确 host / `*.example.com`；空列表允许公网、拒绝内部地址，非空后只允许名单 host，见 §6.34 |
| `model.egress` | allowed-hosts([])，环境变量 `CUSTOMER_WORK_MODEL_EGRESS_ALLOWED_HOSTS`；运行时配置启用的生产环境强制非空，主/备/路由/实验端点构建前统一校验，载荷不能扩大该部署级边界，见 §6.34 |
| `admin.model-health`（非 `customer-work.*` 前缀） | enabled(false), scan-interval-ms(60000), batch-size(20), worker-count(8), queue-capacity(32), probe-timeout-seconds(10), failure-threshold(3), recovery-threshold(2), probe-interval-seconds(300), cooldown-seconds(60), max-override-hours(24)；探测使用有界执行器，超时从实际获得工作线程后计算；生产开启时要求运行时发布同时启用，手工健康探测不受总闸影响，见 §6.34 |

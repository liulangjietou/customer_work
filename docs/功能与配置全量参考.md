...
- **出站必须过流式增量，不能只改最终结果**：接入层（admin `ChatService` / 8080 SSE）把
  `TextBlockDeltaEvent` 逐片推给前端，正文流完之后最终的 `AgentResultEvent` **会被直接丢弃**。
  中间件因此对增量事件做**滑动缓冲**过滤：每片与缓冲区拼接后整体匹配，只放行"确定不再是某个词前缀"
  的前半段，尾部留 `最长词长-1` 个字符等下一片——否则 `阿根廷` 被拆进三次推送时永远命中不了。
  块结束时 flush 尾巴，一个字都不吞。

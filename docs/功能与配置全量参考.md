...
| **SLO / error budget 与业务结果** | `SloEvaluationService` + `BusinessOutcomeService` | 同步触发评估 | 短/长窗口 burn rate、错误预算、技术成功/自动解决代理/转人工/CSAT 会话下钻；见 §6.34 |
...
- 远程 MCP URL 仅接受 http/https，并禁止 userInfo、query、fragment；凭据必须从 URL 移出，当前放在 `headers`，
  完成 SecretRef 迁移后再使用引用。存量非法 URL 的编辑详情按无效配置 fail-closed；最终客户端构建和运行时发布
...
- 当前 `ai_mcp.config` 及发布到 Nacos 的 MCP headers 尚未 SecretRef 化。页面不回显不等于存储层已经密文化；
  生产必须依赖数据库/Nacos 最小权限 ACL、TLS、备份访问控制与审计，后续再把 MCP 凭据迁入 SecretRef。
...
| `/api/aiconfig/model-experiments/**` | 双臂实验创建/启动/停止、事件与真实调用指标；desired/effective state 分列 |
| `/api/aiconfig/runtime-publish/gate/**` | Eval release gate 策略、任务判定、重评与绑定候选 hash 的审计豁免 |
| `GET /api/agent-call-stats/{id}/replay-manifest` | 只读重放清单（`executable=false`），冻结调用输入、输出与运行时/制品版本 |
| `/api/slo/**` · `/api/business-outcomes/**` | SLO/error budget 同步评估；业务结果汇总与会话下钻，见 §6.34 |
...
| `/api/aiconfig/skill` | Skill 管理 CRUD；编辑后自动失效引用它的智能体运行时缓存，`/{id}/download` 导出技能包 zip（`skill:export`） |
...
这解决的是配置漂移、传输损坏和误写检测，**不是数字签名**：具备 Nacos dataId 写权限的主体可以修改正文后
同步重算 hash，因此 Nacos ACL、TLS 与写权限审计仍是发布来源的信任边界。
...
`APPLIED` 由 ACK 数量阈值计算，`admin.runtime-publish.minimum-ack-count` 默认值为 1；任务创建时没有冻结
目标实例清单，因此它不等价于“发布时的所有 Pod 均已确认”。生产多副本必须按现网副本显式设置阈值并监控
`PARTIAL`，需要全实例证明时还应引入实例清单/quorum。
...
每次评测执行前固化不可变用例序列与内容 hash，运行事实绑定评测集、模型、提示词、Agent、知识库、工具、
Judge 和 rubric 九类版本。运行时发布门禁支持指标阈值、相对基线回归、关键用例与 Judge 故障策略；
`GET /api/aiconfig/runtime-publish/gate/tasks/{taskId}` 查看判定，失败任务可重评，紧急豁免必须绑定 taskId 与
候选内容 hash，并记录操作人、原因与原判定。模型实验 ACTIVATE 当前去除实验身份后评测基线候选，再叠加
两臂模型认证与线上错误率/P95 护栏；它不是“双臂均完成离线实验评测”的证明。
...
| FinOps | `/api/billing/forecast`、`/alerts`、`/alerts/{id}/ack`、`/export`；token 实时配额（`BLOCK/DEGRADE/WARN`）与 T+1 金额账单并存 | 金额预测是简单线性外推，无季节性和置信区间；业务结果暂无会话级金额 |
| EvalOps / release gate | 不可变评测集快照与内容 hash、九维版本绑定、阈值/回归/关键用例/Judge 故障门禁、重评和绑定候选 hash 的审计豁免 | 实验 ACTIVATE 当前评测去除实验身份后的基线候选，不是双臂离线效果对比 |
| ModelOps | `/aiconfig/model` 管理资产与部署；SecretRef、端点保存期出网校验、健康快照/事件、上线认证、引用影响分析；不可变路由版本、校验、dry-run 与真实选路 | 当前 SecretRef 仅实现 `LOCAL_AES`；固定 DNS/禁重定向只覆盖连通性/健康探测及认证的连通性采样，AgentScope 厂商 SDK 的认证能力检查与实际推理 HTTP/DNS 仍需后续收口 |
| MCP 治理 | 分页不回 config；编辑详情用 `mcp:edit` 并返回可合并的脱敏占位符；只读探测用 `mcp:view`，真正 `debug/call` 用 `mcp:edit` 且审计 | MCP config 与 Nacos headers 尚未 SecretRef 化，生产依赖数据库/Nacos ACL、TLS 与备份访问控制 |
| Trace / Replay | 调用日志冻结 traceId、runtime revision/hash、模型/提示词/Agent/知识/工具版本和实验曝光；`GET /api/agent-call-stats/{id}/replay-manifest` | Replay 是 `executable=false` 的只读清单，不会再次执行模型或工具 |
| 在线实验 | 不可变双臂、稳定分桶、曝光归因、调用日志指标、最小样本、错误率/P95 护栏、到期/自动停止；desired state 与 effective state 分列 | ACK 默认阈值为 1，发布任务未冻结目标实例清单；`PARTIAL` 按失败处理，不能假定全 Pod 一致 |
| SLO / error budget | `/api/slo/policies` 配置，`POST /api/slo/policies/{id}/evaluate` 同步计算短/长窗、剩余错误预算和 burn rate | 当前不是自动周期评估；无样本返回 `NO_DATA` |
| 业务结果—成本 | `/api/business-outcomes/summary` 与 `/sessions` 汇总真实调用会话的技术成功、自动解决代理、转人工、token 可用性和 CSAT | 金额事实只有“租户 + 日期 + 模型”粒度，cost 返回 `UNAVAILABLE`，不做伪分摊 |
...
| `admin.runtime-publish`（非 `customer-work.*` 前缀） | scan-interval-ms(5000), lease-ms(60000), batch-size(20), max-attempts(8), base-backoff-ms(5000), minimum-ack-count(1), ack-identities([]), nacos.{enabled(false),server-addr,namespace,group,data-id,username,password,timeout-ms}；生产每个实例配置独立的 tenantId/instanceId/至少 32 字节 token，多 Pod 必须按实例数评估 ACK 阈值，见 §6.30、§6.34 |
...
| `admin.model-health`（非 `customer-work.*` 前缀） | enabled(false), scan-interval-ms(60000), batch-size(20), worker-count(8), queue-capacity(32), probe-timeout-seconds(10)；探测使用有界执行器，超时从实际获得工作线程后计算；默认不自动产生外部模型调用费用，手工健康探测不受该开关影响，见 §6.34 |

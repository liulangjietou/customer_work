...
| 类别 | 扩展名 | 解析方式 |
|---|---|---|
| 图片 | png / jpg / jpeg / bmp / webp | 视觉大模型 OCR（`VisionOcrParser` 委托 `ModelVisionOcrService`，不用 tesseract） |
...
| `customer-work.attachment.ocr.provider` | `dashscope` | 视觉 OCR 模型厂商 |
| `customer-work.attachment.ocr.model-name` | `qwen-vl-max` | 视觉 OCR 模型名（通义千问 VL） |
| `customer-work.attachment.ocr.api-key` | （空） | 视觉模型 API Key；留空回落 `customer-work.model.api-key`，再回落环境变量 `DASHSCOPE_API_KEY` |
| `customer-work.attachment.ocr.base-url` | （空） | 自定义端点，留空用 SDK 默认地址 |
| `customer-work.attachment.ocr.timeout-seconds` | `60` | OCR 调用超时（秒） |
| `customer-work.attachment.ocr.prompt` | 见下 | OCR 提示词，默认："请逐字提取这张图片中的全部文字内容，保持原有排版结构，以 Markdown 格式输出，不要添加任何解释说明。" |

> **OCR 视觉模型说明**：图片走视觉大模型识别（不用 tesseract），默认厂商 `dashscope`、模型

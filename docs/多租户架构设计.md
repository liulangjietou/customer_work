...
| 平台级（忽略过滤） | `sys_permission`、`sys_menu_change_log`、`ai_system_tool`、`sys_tenant`、`ai_model_price`、`login_carousel_image`、`flyway_schema_history` | 不加列，进忽略清单 |
...
> 这份表格与代码里的 `TenantInterceptors.PLATFORM_LEVEL_TABLES` 必须逐条对齐。
...
实现在 **`ModelConfigService`**（`applyReadScope` / `requireModel` / `requireWritableModel` / `clearOtherDefaults`）：

| 操作 | 控制 |
|---|---|
| 列表 / 详情 | `tenant_id IN (当前租户, '__platform__')`；不可见与不存在统一报 404，不泄漏"这个 id 属于别人" |
| 平台记录的凭据回显 | 对非运营方置为固定占位串（脱敏串仍会漏出前后若干位，而这把 key 不属于他们） |
| 更新 / 删除 | 强制归属本租户，租户改不动平台记录 |
...
> **这段补偿控制是这张表唯一的一道防线**——它刻意不在 `DataScopeTables` 白名单里（见 `docs/数据权限设计.md`），
> 没有第二层兜底。删掉 `ModelConfigService` 里的任何一条，忽略清单那边都不会报错。
>
...
配额表带 `tenant_id`，但**刻意不参与自动过滤**：运营方要跨租户配额度，而租户管理员本就不该看到自己的额度设置
（能看到就离能改不远）。这与 §2.4 的方向相反——那张表是"放宽读、收紧写"，这张表是"整体不给租户看"。

访问控制因此完全落在 **Controller 的运营方校验**上。**给这张表新写任何查询接口，都必须显式加运营方校验**：

-- =============================================================================
-- XXL-JOB 调度中心 · 建库建表脚本（官方原版，未做任何改动）
-- =============================================================================
-- 说明：
--   1. 与本项目其余库（agent_scope_customer_work / customer_admin）物理隔离，独立数据库 `xxl_job`。
--   2. 内容原样取自 XXL-JOB 官方仓库 3.3.2 tag 下的 doc/db/tables_xxl_job.sql
--      （https://github.com/xuxueli/xxl-job/blob/3.3.2/doc/db/tables_xxl_job.sql）——
--      与本项目 pom 里声明的 xxl-job-core 版本（3.3.2）严格对应，避免调度中心/执行器协议不一致。
--   3. 末尾 xxl_job_group/xxl_job_info 是官方自带的示例执行器/示例任务种子数据（job_group=2 那条
--      还带了 ollamaJobHandler/difyWorkflowJobHandler 两个跟本项目无关的示例 JobHandler），

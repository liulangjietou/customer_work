...
-- Projects：跨智能体的会话分组。会话本身没有自增主键，关联表用 (agent_code, session_id) 复合列。
...
-- ---- 默认权限树 ----
-- 一级：系统管理 / AI 配置 / 智能体工作区 / Projects（type=1 菜单，parent_id=0）
-- Projects 无子节点，MenuTree.vue 按 children 是否为空自动渲染成直接可点的叶子菜单；权限点复用
-- workspace（跟"智能体工作区"同一使用场景，不新增 project:* 按钮权限）。
INSERT INTO `sys_permission` (`id`, `parent_id`, `perm_name`, `perm_code`, `type`, `path`, `icon`, `sort`) VALUES
    ...
    (4,  0, 'Projects',    'project',   1, '/project',   'Files',       4);
